...
def get_statistics(config, assetdir, mode):
    from INDM.evaluation_original import load_dataset_stats
    stats = load_dataset_stats(config, assetdir)
    ...
    # compute fid of a folder
        ...
        for sampling_itr in sampling_itrs:
            score = fid_folder(fdir1, dataset, dataset_name, dataset_res, dataset_split,
                ...
                batch_size=batch_size, device=device, sampling_itr=sampling_itr, assetdir=assetdir, config=config, dequantization=True,
                               num_data=num_data)
